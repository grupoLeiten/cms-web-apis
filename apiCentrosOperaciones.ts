...
export const getCentrosOperaciones = async ({ token }: { token: string }) : Promise<{dataGetCentrosOperaciones: any}> => {
    ...
    const result = centrosOperaciones === undefined ? [{ telefono: "1", nombre: "1", horario: "1" }] : centrosOperaciones
    return { dataGetCentrosOperaciones: result }
}